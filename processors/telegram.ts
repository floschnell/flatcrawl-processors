...
let searches: Map<string, Search> = new Map();
...
telegraf.command(['search'], ctx => {
  ...
  return ctx
    ...
    .then(() =>
      ctx.reply(
        `First, tell me about your budget. How many Euro would you like to spent per month? (eg. "400-800" or "-1000" for no minimum)`,
        question(ctx)
      )
    );
});
...
telegraf.command(['subscriptions'], ctx => {
  dbConnection.getSubscriptionsForChat(ctx.chat.id).then(subscriptions => {
    if (subscriptions) {
      const subscriptionKeys = Object.keys(subscriptions).map(
  ...
  if (ctx.session.intent === 'create') {
    ...
    if (stepName === 'limit') {
      ...
      if (!limits || (limits[1] === undefined && limits[2] === undefined)) {
        return ctx.reply(
          'Sorry, I did not understand. Please use the format "[min] - [max]".',
          question(ctx)
        );
      } else {
        const limit = {};
        ...
        return ctx.reply(msg.join('\n'), question(ctx, false)).then(() => {
          if (limitName === 'rent') {
            ...
            ctx.reply(
              'How many squaremeters would you like the flat to have?',
              question(ctx)
            );
          } else if (limitName === 'squaremeters') {
            ctx.session.step = 'limit:rooms';
            ctx.reply(
              'How many rooms would you want the flat to have?',
              question(ctx)
            );
            ...
            ctx
              ...
              .then(() =>
                ctx.reply(
                  'Do you want to add a location right now? (yes/no)',
                  question(ctx)
                )
              );
          }
      ...
      if (substep === 'continue') {
        ...
        if (response === 'yes') {
          ...
          ctx.reply(
            'Please send me the address of the location that you would like to receive the distance for.',
            question(ctx)
          );
        } else if (response === 'no') {
        ...
        Directions.getCoordsForAddress(
        ...
        if (response === 'yes') {
          ...
          ctx.session.step = 'locations:add';
          ctx.reply(
            'Okay, then try to send me a more accurate address.',
            question(ctx)
          );
        } else {
          ctx.reply('Sorry, I did not understand. Yes or no?', question(ctx));
        ...
        if (chosenMode.length === 0) {
          ctx.session.step = 'locations:finish';
          ctx.reply(
            `I do not know the mode ${receivedMode}, please choose between: ${supportedTransportModes
          ...
          ctx
            ...
            .then(() =>
              ctx.reply(
                'Would you like to add another location? (yes/no)',
                question(ctx)
              )
            );
        }
...
function calculateDirections(search, flat) {
  return Directions.getCoordsForAddress(flat.address).then(flatGeo =>
    Promise.all(
  ...
  telegram
    ...
    .then(chat => {
      ...
      directions.forEach(direction => {
        message.push(
          `From this flat to *${direction.name}* will take you *${direction.duration}* (${direction.distance}) by ${direction.transport}`
        );
      });
